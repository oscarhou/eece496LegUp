...
typedef unsigned long long Lane;
/*
Round Constants
*/
...
Lane rotateLeft(Lane lane, int rotateCount)
{
    ...
    Lane LeftShiftValue = ((Lane)lane << rotate);
    Lane RightShiftValue = ((Lane)lane >> (LANE_BIT_SIZE - rotate));
    Lane XORValue =  ((Lane)lane << rotate) xor ((Lane)lane >> (LANE_BIT_SIZE - rotate)) ; 
    Lane ORValue =  ((Lane)lane << rotate) | ((Lane)lane >> (LANE_BIT_SIZE - rotate)) ; 
    printf("rotateAmount = %d\n", rotate);
    printf("lane << rotate = %llu\n", LeftShiftValue);
    printf("lane >> LANE_BIT_SIZE - rotate = %llu\n", RightShiftValue);
    printf("((Lane)lane << rotate) | ((Lane)lane >> (LANE_BIT_SIZE - rotate)) = %llu\n", ORValue);
    printf("((Lane)lane << rotate) ^ ((Lane)lane >> (LANE_BIT_SIZE - rotate)) = %llu\n", XORValue);
...
void roundFunction (Lane A[][ARRAY_WIDTH])
{
	...
	//printf("C Value  After theta :%llx, %llx, %llx, %llx, %llx\n", C[0], C[1],C[2],C[3],C[4]);
    for (x = 0; x < ARRAY_WIDTH; x++)
    {
        D[x] = (rotateLeft(C[(x+1) % 5], 1) ^ C[(x+4) % 5]) ;
    }
	//printf("D Value: %llx, %llx, %llx, %llx, %llx\n", D[0], D[1],D[2],D[3],D[4]);
